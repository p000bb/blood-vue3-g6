...
export const handleDownloadImage = (graph: any) => {
  if (!graph) return;
  graph.downloadFullImage("血缘图", "image/png", {
    padding: [30, 15, 15, 15],
  });
};
 ...
 * @param color
 */
export const handleNodeColor = (graph: any, level: number, color: string) => {
  if (!graph) return;
  const nodes = graph.getNodes();
  if (nodes) {
    nodes.forEach((node: any) => {
      const nodeLevel = node.getModel().level;
      if (nodeLevel === level) {
        graph.setItemState(node, `fill-${color}`, true);
      }
    });
  }
};
